...
------------- modify
    self.json_exist = is_exist(dataFile)
    self.json_default_used = false
    if not self.json_exist then 
      dataFile = "defaults/animation/_DEFAULT.json" 
      self.json_default_used = true
    end 
...
function peachy:_checkImageSize()
  ...
  ------------- modify
  if self.json_default_used == true then
    self._jsonData.meta.size.w = self.image:getWidth()
    self._jsonData.meta.size.h = self.image:getHeight()
  end
  -------------
